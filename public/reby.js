...
function startSlideshow() {
  stopSlideshow();
  if (slideA) slideA.style.backgroundImage = `url('${privateImages[0]}')`;
  if (slideB) slideB.style.backgroundImage = `url('${privateImages[1]}')`;
  slideIndex = 1;
  if (slideA) slideA.classList.add("top");
  if (slideB) slideB.classList.remove("top");
  showingA = true;
  slideInterval = setInterval(() => {
    const next = privateImages[(slideIndex + 1) % privateImages.length];
    if (showingA) {
      if (slideB) slideB.style.backgroundImage = `url('${next}')`;
      if (slideB) slideB.classList.add("top");
      if (slideA) slideA.classList.remove("top");
    } else {
      if (slideA) slideA.style.backgroundImage = `url('${next}')`;
      if (slideA) slideA.classList.add("top");
      if (slideB) slideB.classList.remove("top");
    }
    showingA = !showingA;
...
function stopSlideshow() {
  ...
  slideInterval = null;
  if (slideA) { slideA.classList.add("top"); slideA.style.backgroundImage = ""; }
  if (slideB) { slideB.classList.remove("top"); slideB.style.backgroundImage = ""; }
}
...
messageInput.addEventListener('input', () => {
  ...
  typingTimer = setTimeout(() => socket.emit('stopTyping', { name: username, room: currentRoom }), 1200);
});
